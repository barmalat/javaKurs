...
public class Exercises {
    ...
    public static Optional<User> getRichestWoman() {
        return users.stream()
                .filter(user -> user.getSex() == Sex.WOMAN).max(Comparator.comparing(user -> getUserAmountInPLN(user)));
    ...
    public static Map<String, List<User>> getUserPerCompany() {
        return holdings
                .stream()
                .flatMap(holding -> holding.getCompanies().stream())
                .collect(Collectors.toMap(company -> String.valueOf(company.getName()), company -> company
                        .getUsers()
                        .stream()
